...
func TestSessionPrefix(t *testing.T) {
	if SessionPrefix != "agentdesk_" {
		t.Errorf("SessionPrefix = %s, want agentdesk_", SessionPrefix)
	}
}
...
func TestReconnectSession(t *testing.T) {
	...
	sess := ReconnectSession("agentdesk_test_abc123", "test", "/tmp", "claude")

	if sess.Name != "agentdesk_test_abc123" {
		t.Errorf("Name = %s, want agentdesk_test_abc123", sess.Name)
	}
...
func TestReconnectSessionHasStateTracker(t *testing.T) {
	sess := ReconnectSession("agentdesk_test_123", "my-project", "/home/user/project", "claude")
...
func TestStateTrackerPointersAreIndependent(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		sessions[i] = ReconnectSession(
			"agentdesk_test_"+string(rune('a'+i)),
...
func TestReconnectSessionLazyDoesNotConfigure(t *testing.T) {
	...
	sess := ReconnectSessionLazy("agentdesk_test_lazy", "lazy-project", "/tmp", "claude", "idle")
	...
	if sess.Name != "agentdesk_test_lazy" {
		t.Errorf("Expected name agentdesk_test_lazy, got %s", sess.Name)
	}
...
func TestEnsureConfiguredIdempotent(t *testing.T) {
	sess := ReconnectSessionLazy("agentdesk_test_idempotent", "test", "/tmp", "claude", "idle")
...
func TestReconnectSessionWithStatusIdle(t *testing.T) {
	...
	sess := ReconnectSessionWithStatus("agentdesk_test_123", "my-project", "/tmp", "claude", "idle")
...
func TestReconnectSessionWithStatusWaiting(t *testing.T) {
	...
	sess := ReconnectSessionWithStatus("agentdesk_test_456", "other-project", "/tmp", "claude", "waiting")
...
func TestReconnectSessionWithStatusActive(t *testing.T) {
	...
	sess := ReconnectSessionWithStatus("agentdesk_test_789", "active-project", "/tmp", "claude", "active")
...
func TestAppRestartPersistenceFlow(t *testing.T) {
	...
	sess := ReconnectSessionWithStatus("agentdesk_project_abc", "project", "/tmp", "claude", "idle")
...
func TestNewSessionsStartYellow(t *testing.T) {
	...
	sess := ReconnectSessionWithStatus("agentdesk_new_xyz", "new-project", "/tmp", "claude", "waiting")
...
func TestSessionLogFile(t *testing.T) {
	...
	assert.Contains(t, logFile, ".agent-desk/logs/")
	assert.Contains(t, logFile, "agentdesk_test-log")
...
func TestSession_SetsUpActivityMonitoring(t *testing.T) {
	if os.Getenv("AGENTDESK_TEST_PROFILE") == "" {
...
func TestSetStatusLeft(t *testing.T) {
	...
	// Create a test session
	sessionName := "agentdesk_test_notification_" + fmt.Sprintf("%d", time.Now().UnixNano())
