...
Add a "Just do it (vagrant sudo)" checkbox to the New Session dialog, below "Teammate mode". When checked, agent-desk auto-manages a Vagrant VM lifecycle and runs Claude Code inside it with `--dangerously-skip-permissions` and sudo access. Based on [Running Claude Code Dangerously Safely](https://blog.emilburzo.com/2026/01/running-claude-code-dangerously-safely/).
...
- [ ] agent-desk crash recovery: reconnects to surviving tmux+VM+Claude automatically
...
- CLI flag support (`agent-desk add --vagrant`) -- TUI only for now
...
- Skill: Static file bundled with agent-desk
...
agent-desk restart ──→ ReconnectSessionLazy() finds tmux session
...
| agent-desk crashes | Automatic recovery on restart via tmux reconnection. No action needed. |
...
MCP tools configured in agent-desk's `config.toml` must work inside the Vagrant VM where Claude Code runs. There are three transport types and three scopes to handle.
...
| Pool socket | `command: "agent-desk", args: ["mcp-proxy", "/tmp/agentdesk-mcp-NAME.sock"]` | Host Unix socket inaccessible from VM |
...
Vagrant sessions introduce failure modes that don't exist with direct tmux sessions. The VM, VirtualBox, or agent-desk itself can crash independently. Recovery must be seamless -- the user should be able to press `R` (restart) and resume where they left off.

#### Failure Scenarios

| Scenario | What Dies | What Survives | Detection | Recovery |
|----------|-----------|---------------|-----------|----------|
| VM crashes (OOM, kernel panic) | VM + Claude | tmux pane (shows exit), agent-desk, SQLite state | `vagrant ssh -c` exits non-zero; `UpdateStatus()` → StatusError | Press R: `vagrant up` → respawn Claude with `--resume` |
| VirtualBox crashes | VM + Claude | tmux pane, agent-desk, SQLite state | Same as above | Press R: same recovery flow |
| agent-desk crashes | TUI process | tmux session (still running `vagrant ssh -c`), VM, Claude | On restart: `ReconnectSessionLazy()` finds tmux session | Automatic: reconnects to existing tmux+VM+Claude |
| agent-desk + VM crash (host reboot) | Everything | SQLite state on disk | On restart: tmux session not found, `vagrant status` → not_created | Press R: `vagrant up` → fresh Claude with `--resume` |
| VM hangs (unresponsive) | Nothing (but frozen) | tmux pane (frozen), agent-desk | `UpdateStatus()` detects no activity for extended period | User kills session, restarts. VM force-destroyed. |
| Shared folder sync breaks | File I/O inside VM | VM, Claude (but erroring), agent-desk | Claude reports file errors in tmux output | Press R: `vagrant reload` (restarts VM, re-mounts folders) |
...
#### agent-desk Crash Recovery

When agent-desk itself crashes, the recovery is largely automatic thanks to existing infrastructure:
...
The only edge case: if agent-desk crashes during `vagrant up` (before Claude launches), the tmux session may show a partial vagrant output. On restart:
...
On startup, if `UseVagrantMode` is true and `cleanShutdown` is false (default for loaded instances), the first `UpdateStatus()` call triggers an immediate VM health check instead of waiting 60s. This catches cases where the host rebooted or agent-desk was killed.
...
- [ ] agent-desk crash + restart: automatically reconnects to running vagrant session
...
- agent-desk crash recovery: kill agent-desk → restart → session auto-reconnects
