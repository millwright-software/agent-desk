...
#
# Options:
#   --keep-data         Keep ~/.agent-desk/ (sessions, config, logs)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dry-run)
            ...
            ;;
        -h|--help)
            echo "Agent Desk Uninstaller"
            echo ""
            echo "Usage: uninstall.sh [options]"
            echo ""
            echo "Options:"
            echo "  --keep-data         Keep ~/.agent-desk/ (sessions, config, logs)"
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║       Agent Desk Uninstaller           ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
echo ""
...
if command -v brew &> /dev/null && brew list agent-desk &> /dev/null 2>&1; then
...
if [[ -f "$TMUX_CONF" ]] && grep -q "# agent-desk configuration" "$TMUX_CONF" 2>/dev/null; then
    FOUND_ITEMS+=("tmux")
...
if [[ ${#FOUND_ITEMS[@]} -eq 0 ]]; then
    echo -e "${YELLOW}Agent Desk does not appear to be installed.${NC}"
    echo ""
    ...
    echo "  - $DATA_DIR"
    echo "  - $TMUX_CONF (for agent-desk config)"
    exit 0
fi
...
for item in "${FOUND_ITEMS[@]}"; do
    case "$item" in
        homebrew)
            echo -e "  ${RED}•${NC} Homebrew package: agent-desk"
            ;;
        binary:*)
            loc="${item#binary:}"
...
if [[ " ${FOUND_ITEMS[*]} " =~ " tmux " ]] && [[ "$KEEP_TMUX_CONFIG" != "true" ]]; then
    ...
    # Create backup
    cp "$TMUX_CONF" "$TMUX_CONF.bak.agentdesk-uninstall"

    # Remove the agent-desk config block (between markers)
    # Using sed to delete from start marker to end marker
    if [[ "$(uname)" == "Darwin" ]]; then
        # macOS sed requires different syntax
        sed -i '' '/# agent-desk configuration/,/# End agent-desk configuration/d' "$TMUX_CONF"
    else
        sed -i '/# agent-desk configuration/,/# End agent-desk configuration/d' "$TMUX_CONF"
    fi
    ...
    echo -e "${GREEN}✓${NC} tmux configuration removed (backup: ~/.tmux.conf.bak.agentdesk-uninstall)"
fi
...
if [[ " ${FOUND_ITEMS[*]} " =~ " data " ]] && [[ "$KEEP_DATA" != "true" ]]; then
    ...
    if [[ "$NON_INTERACTIVE" != "true" ]]; then
        ...
        echo
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
            BACKUP_FILE="$HOME/agent-desk-backup-$(date +%Y%m%d-%H%M%S).tar.gz"
            echo -e "Creating backup at $BACKUP_FILE..."
            tar -czf "$BACKUP_FILE" -C "$HOME" .agent-desk
...
if [[ "$KEEP_DATA" == "true" ]]; then
    ...
    echo "      Remove manually with: rm -rf ~/.agent-desk"
fi

if [[ "$KEEP_TMUX_CONFIG" == "true" ]]; then
    echo -e "${YELLOW}Note:${NC} tmux config preserved in ~/.tmux.conf"
    echo "      Remove the '# agent-desk configuration' block manually if desired"
fi

echo ""
echo "Thank you for using Agent Desk!"
echo "Feedback: https://github.com/millwright-software/agent-desk/issues"
