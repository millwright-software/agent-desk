...
**Version:** 1.0.10 | **Repo:** [github.com/millwright-software/agent-desk](https://github.com/millwright-software/agent-desk) | **Issues:** [github.com/millwright-software/agent-desk/issues](https://github.com/millwright-software/agent-desk/issues)
...
```
Base directory for this skill: /path/to/.../skills/agent-desk
```
...
**For plugin users**, the path looks like: `~/.claude/plugins/cache/agent-desk/agent-desk/<hash>/skills/agent-desk/scripts/`
**For local development**, the path looks like: `<repo>/skills/agent-desk/scripts/`
...
agent-desk add -t "Project" -c claude /path/to/project
agent-desk session start "Project"

# Send message and get output
agent-desk session send "Project" "Analyze this codebase"
agent-desk session output "Project"
```
...
| Command | Purpose |
|---------|---------|
| `agent-desk` | Launch interactive TUI |
| `agent-desk add -t "Name" -c claude /path` | Create session |
| `agent-desk session start/stop/restart <name>` | Control session |
...
| `agent-desk add --worktree <branch>` | Create session in git worktree |
| `agent-desk worktree list` | List worktrees with sessions |
| `agent-desk worktree cleanup` | Find orphaned worktrees/sessions |
...
| **On-demand** | `agent-desk session output "Title"` | User asks to check |
...
**DO NOT** try to create Codex/Gemini sessions manually with `agent-desk add`. Always use the script above. It handles tool-specific initialization, readiness detection, and output retrieval automatically.
...
6. Without `--wait`: returns immediately, check output later with `agent-desk session output "Title"`
...
```bash
agent-desk remove "Consult Codex"
# Or remove multiple at once:
agent-desk remove "Codex Review" && agent-desk remove "Gemini Arch"
```
...
# Attach and restart
agent-desk mcp attach <session> <mcp-name>
agent-desk session restart <session>

# Or attach on create
agent-desk add -t "Task" -c claude --mcp exa /path
```
...
agent-desk add /path/to/repo -t "Feature Work" -c claude --worktree feature/my-feature --new-branch

# Create session in existing branch's worktree
agent-desk add . --worktree develop -c claude
```
...
agent-desk worktree list

# Show detailed info for a session's worktree
agent-desk worktree info "My Session"

# Find orphaned worktrees/sessions (dry-run)
agent-desk worktree cleanup

# Actually clean up orphans
agent-desk worktree cleanup --force
```
...
cat ~/.agent-desk/config.toml | grep -v "KEY\|TOKEN\|SECRET"  # Sanitized config

# Create issue at:
# https://github.com/millwright-software/agent-desk/issues/new
```
...
4. Relevant log: `tail -100 ~/.agent-desk/logs/agentdesk_<session>_*.log`
