...
CURRENT_JSON=$(agent-desk session current --json 2>/dev/null | grep -v '^20')
...
if [ -z "$PARENT" ] || [ "$PARENT" = "null" ]; then
    echo "Error: Not in an agent-desk session" >&2
    exit 1
fi
...
# Build add command
ADD_CMD="agent-desk -p $PROFILE add -t \"$TITLE\" --parent \"$PARENT\" -c $TOOL"
for mcp in "${MCPS[@]}"; do
...
eval "$ADD_CMD"
agent-desk -p "$PROFILE" session start "$TITLE"

# Get tmux session name for readiness check
TMUX_SESSION=$(agent-desk -p "$PROFILE" session show "$TITLE" 2>/dev/null | grep '^Tmux:' | awk '{print $2}')
...
# Send prompt
agent-desk -p "$PROFILE" session send "$TITLE" "$PROMPT"
...
echo ""
echo "Check output with: agent-desk session output \"$TITLE\""
...
if [ "$WAIT" = "true" ]; then
    ...
    START_TIME=$(date +%s)
    while true; do
        STATUS=$(agent-desk -p "$PROFILE" session show "$TITLE" 2>/dev/null | grep '^Status:' | awk '{print $2}')
        ...
        if [ "$STATUS" = "◐" ] || [ "$STATUS" = "waiting" ]; then
            ...
            if ! agent-desk -p "$PROFILE" session output "$TITLE" 2>/dev/null; then
        ...
        if [ $ELAPSED -ge $TIMEOUT ]; then
            ...
            echo "Check later with: agent-desk session output \"$TITLE\""
            exit 1
        fi
