...
All options for `~/.agent-desk/config.toml`.
...
**Logs location:** `~/.agent-desk/logs/agentdesk_<session>_<id>.log`
...
**Socket location:** `/tmp/agentdesk-mcp-{name}.sock`
...
- CLI: `agent-desk mcp server status/start/stop`
...
**Built-in icons:** claude=🤖, gemini=✨, opencode=🌐, codex=💻, cursor=📝, copilot=🐙, shell=🐚
...
| Variable | Purpose |
|----------|---------|
| `AGENTDESK_PROFILE` | Override default profile |
| `CLAUDE_CONFIG_DIR` | Override Claude config dir |
| `AGENTDESK_DEBUG=1` | Enable debug logging |
