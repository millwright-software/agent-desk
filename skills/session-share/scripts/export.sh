...
if [ -z "$SESSION_ID" ]; then
    ...
    CURRENT_JSON=$(agent-desk session current --json 2>/dev/null | grep -v '^20' || echo "{}")
    ...
    if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
        ...
        echo "Make sure you're in an agent-desk session with an active Claude conversation." >&2
        exit 1
    fi
else
