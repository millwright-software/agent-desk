...
CURRENT_OUTPUT=$(agent-desk session current --json 2>&1 || true)
...
# Create agent-desk session
echo ""
echo "Creating agent-desk session..."
...
EXISTING=$(agent-desk -p "$PROFILE" list --json 2>/dev/null | jq -r --arg title "$IMPORT_TITLE" '.[] | select(.title == $title) | .id' || echo "")

if [ -n "$EXISTING" ]; then
    echo "Session '$IMPORT_TITLE' already exists. Updating..."
    # Just update the session to point to the new session ID
    agent-desk -p "$PROFILE" session set "$IMPORT_TITLE" claude-session-id "$SESSION_ID"
else
    # Create new session
    agent-desk -p "$PROFILE" add -t "$IMPORT_TITLE" -c claude "$PROJECT_PATH"

    # Poll for session creation (up to 10 attempts, 0.5s each)
    SESSION_FOUND=false
    for i in {1..10}; do
        if agent-desk -p "$PROFILE" list --json 2>/dev/null | jq -e --arg title "$IMPORT_TITLE" '.[] | select(.title == $title)' > /dev/null 2>&1; then
            SESSION_FOUND=true
            break
        fi
        sleep 0.5
...
if [ "$NO_START" = "true" ]; then
    ...
    echo ""
    echo "To start: agent-desk session start \"$IMPORT_TITLE\""
    echo "Or: Open agent-desk TUI and press Enter on the session"
else
    echo "Starting session..."
    agent-desk -p "$PROFILE" session start "$IMPORT_TITLE"

    echo ""
    echo "Session is now running. It will resume from the imported conversation."
    echo ""
    echo "To attach: agent-desk session attach \"$IMPORT_TITLE\""
    echo "Or: Open agent-desk TUI and press Enter on the session"
fi
