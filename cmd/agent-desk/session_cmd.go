...
func printSessionHelp() {
	fmt.Println("Usage: agent-desk session <command> [options]")
	fmt.Println()
	...
	fmt.Println("  agent-desk session set-parent sub-task main-project  # Make sub-task a sub-session")
	fmt.Println("  agent-desk session unset-parent sub-task             # Remove sub-session link")
	fmt.Println("  agent-desk session output my-project                 # Get last response from session")
	fmt.Println("  agent-desk session output my-project --json          # Get response as JSON")
	fmt.Println()
	...
	fmt.Println("  agent-desk session set my-project title \"New Title\"")
	fmt.Println("  agent-desk session set my-project claude-session-id \"abc123-def456\"")
	fmt.Println("  agent-desk session set my-project tool claude")
	fmt.Println("  agent-desk session set my-project wrapper \"nvim +'terminal {command}'\"")
}
...
func handleSessionStart(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session start <id|title> [options]")
		fmt.Println()
		...
		fmt.Println("Examples:")
		fmt.Println("  agent-desk session start my-project")
		fmt.Println("  agent-desk session start my-project --message \"Research MCP patterns\"")
		fmt.Println("  agent-desk session start my-project -m \"Explain this codebase\"")
	}
...
func handleSessionStop(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session stop <id|title> [options]")
		fmt.Println()
...
func handleSessionRestart(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session restart <id|title> [options]")
		fmt.Println()
...
func handleSessionAttach(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session attach <id|title>")
		fmt.Println()
...
func handleSessionShow(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session show [id|title] [options]")
		fmt.Println()
...
func handleSessionSet(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session set <id|title> <field> <value> [options]")
		fmt.Println()
		...
		fmt.Println("  agent-desk session set my-project title \"New Title\"")
		fmt.Println("  agent-desk session set my-project claude-session-id \"abc123-def456\"")
		fmt.Println("  agent-desk session set my-project path /new/path/to/project")
		fmt.Println("  agent-desk session set my-project wrapper \"nvim +'terminal {command}'\"")
	}
...
func findSessionByTmux(instances []*session.Instance) *session.Instance {
	...
	// Parse agent-desk session name: agentdesk_<title>_<id>
	if strings.HasPrefix(sessionName, "agentdesk_") {
		// Extract title (everything between agentdesk_ and the last _id)
		withoutPrefix := strings.TrimPrefix(sessionName, "agentdesk_")
		...
		if lastUnderscore > 0 {
			...
			// For agentdesk sessions, we have the title - don't fall back to path matching
			// as that could match a different session with same path in another profile
			return nil
		}
	}

	// Try to find by path (only for non-agentdesk tmux sessions)
	for _, inst := range instances {
...
func showTmuxSessionInfo(out *CLIOutput, jsonOutput bool) {
	...
	var sb strings.Builder
	sb.WriteString("⚠ Session not registered in agent-desk\n")
	sb.WriteString(fmt.Sprintf("Tmux:    %s\n", sessionName))
	...
	sb.WriteString(fmt.Sprintf("  agent-desk add -t \"%s\" -g <group> -c claude %s\n", title, currentPath))
...
func handleSessionSetParent(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session set-parent <session> <parent>")
		fmt.Println()
...
func handleSessionUnsetParent(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session unset-parent <session>")
		fmt.Println()
...
func handleSessionSend(profile string, args []string) {
	...
	if len(remaining) < 2 {
		out.Error("usage: agent-desk session send <id> <message>", ErrCodeInvalidOperation)
		os.Exit(1)
	}
...
func handleSessionOutput(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session output [id|title] [options]")
		fmt.Println()
...
func handleSessionCurrent(profileArg string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk session current [options]")
		fmt.Println()
	...
	if instData == nil {
		out.Error(
			"current tmux session is not an agent-desk session\nHint: Run 'agent-desk list' to see available sessions",
			ErrCodeNotFound,
		)
		os.Exit(1)
	}
