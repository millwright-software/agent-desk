...
// GetCurrentSessionID detects the current agent-desk session from tmux environment
// Returns session ID or empty string if not in an agent-desk session
...
func ResolveSessionOrCurrent(identifier string, instances []*session.Instance) (*session.Instance, string, string) {
	if identifier == "" {
		...
		if currentID == "" {
			return nil, "no session specified and not inside an agent-desk session", ErrCodeNotFound
		}
