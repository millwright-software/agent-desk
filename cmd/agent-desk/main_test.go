...
func TestNestedSessionAllowsCLICommands(t *testing.T) {
	...
	// Non-agentdesk tmux session should not be detected as nested
	t.Run("non_agentdesk_tmux", func(t *testing.T) {
		orig := os.Getenv("TMUX")
		...
		// The key invariant is: a non-agentdesk session name returns "".
		// We verify this by checking the helper logic directly.
		id := GetCurrentSessionID()
		// In CI/test, either tmux isn't running or we're not in an agentdesk session
		if id != "" {
