...
import (
	...
	"github.com/millwright-software/agent-desk/internal/ui"
...
func printUpdateNotice() {
	...
	fmt.Fprintf(os.Stderr, "\n💡 Update available: v%s → v%s (run: agent-desk update)\n",
...
func promptForUpdate() bool {
	...
	if response != "" && response != "y" && response != "yes" {
		fmt.Println("Skipped. Run 'agent-desk update' later.")
		return false
	}
	...
	fmt.Println("Restart agent-desk to use the new version.")
	return true
}
...
func initColorProfile() {
	...
	// AGENTDESK_COLOR: truecolor, 256, 16, none
	if colorEnv := os.Getenv("AGENTDESK_COLOR"); colorEnv != "" {
...
func main() {
	...
	if len(args) > 0 {
		...
		case "mcp-proxy":
			if len(args) < 2 {
				fmt.Fprintln(os.Stderr, "Usage: agent-desk mcp-proxy <socket-path>")
				os.Exit(1)
			}
			runMCPProxy(args[1])
			return
	...
	if isNestedSession() {
		fmt.Fprintln(os.Stderr, "Error: Cannot launch the agent-desk TUI inside an agent-desk session.")
		...
		fmt.Fprintln(os.Stderr, "  agent-desk add /path -t \"Title\"    # Add a new session")
		fmt.Fprintln(os.Stderr, "  agent-desk session start <id>      # Start a session")
		fmt.Fprintln(os.Stderr, "  agent-desk mcp attach <id> <mcp>   # Attach MCP")
		fmt.Fprintln(os.Stderr, "  agent-desk list                    # List sessions")
		fmt.Fprintln(os.Stderr, "")
	...
	if _, err := exec.LookPath("tmux"); err != nil {
		...
		fmt.Println("\nAgent Desk requires tmux. Install with:")
	...
	if !instanceSettings.GetAllowMultiple() {
		if db := statedb.GetGlobal(); db != nil {
			...
			if electErr == nil && !isFirst {
				fmt.Println("Error: agent-desk is already running for this profile")
	...
	// When AGENTDESK_DEBUG is set, logs go to ~/.agent-desk/debug.log
	// When not set, logs are discarded to avoid TUI interference
	debugMode := os.Getenv("AGENTDESK_DEBUG") != ""
	if baseDir, err := session.GetAgentDeskDir(); err == nil {
...
func handleAdd(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk add [path] [options]")
		fmt.Println()
		fmt.Println("Add a new session to Agent Desk.")
		fmt.Println()
		...
		fmt.Println("  agent-desk add                       # Use current directory")
		fmt.Println("  agent-desk add /path/to/project")
		fmt.Println("  agent-desk add -t \"My Project\" -g \"work\"")
		fmt.Println("  agent-desk add -c claude .")
		fmt.Println("  agent-desk -p work add               # Add to 'work' profile")
		fmt.Println("  agent-desk add -t \"Sub-task\" --parent \"Main Project\"  # Create sub-session")
		fmt.Println("  agent-desk add -t \"Research\" -c claude --mcp memory --mcp sequential-thinking /tmp/x")
		fmt.Println("  agent-desk add -c opencode --wrapper \"nvim +'terminal {command}' +'startinsert'\" .")
		fmt.Println("  agent-desk add --quick -c claude .   # Auto-generated name")
		fmt.Println()
		fmt.Println("Worktree Examples:")
		fmt.Println("  agent-desk add -w feature/login .    # Create worktree for existing branch")
		fmt.Println("  agent-desk add -w feature/new -b .   # Create worktree with new branch")
		fmt.Println("  agent-desk add --worktree fix/bug-123 --new-branch /path/to/repo")
	}
	...
	if wtBranch != "" {
		...
		if _, err := os.Stat(worktreePath); err == nil {
			...
			fmt.Fprintf(os.Stderr, "Tip: Use 'agent-desk add %s' to add the existing worktree\n", worktreePath)
			os.Exit(1)
		}
	...
	humanLines = append(humanLines, fmt.Sprintf("  agent-desk session start %s   # Start the session", sessionTitle))
	humanLines = append(humanLines, "  agent-desk                         # Open TUI and press Enter to attach")
...
func handleList(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk list [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  agent-desk list                    # List from default profile")
		fmt.Println("  agent-desk -p work list            # List from 'work' profile")
		fmt.Println("  agent-desk list --all              # List from all profiles")
	}
...
func handleRemove(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk remove <id|title>")
		fmt.Println()
		fmt.Println("Remove a session by ID or title.")
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  agent-desk remove abc12345")
		fmt.Println("  agent-desk remove \"My Project\"")
		fmt.Println("  agent-desk -p work remove abc12345   # Remove from 'work' profile")
	}
	...
	if err := inst.Kill(); err != nil {
		...
		if inst.Exists() && !*jsonOutput {
			...
			fmt.Println("Session removed from Agent Desk but may still be running in tmux")
		}
	}
...
func handleStatus(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk status [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  agent-desk status              # Quick summary")
		fmt.Println("  agent-desk status -v           # Detailed list")
		fmt.Println("  agent-desk status -q           # Just waiting count")
		fmt.Println("  agent-desk -p work status      # Status for 'work' profile")
	}
...
func handleProfile(args []string) {
	...
	case "create", "new":
		if len(filteredArgs) < 2 {
			...
			if !jsonMode {
				fmt.Println("Usage: agent-desk profile create <name>")
			}
			os.Exit(1)
		}
	...
	case "delete", "rm":
		if len(filteredArgs) < 2 {
			out.Error("profile name is required", ErrCodeInvalidOperation)
			if !jsonMode {
				fmt.Println("Usage: agent-desk profile delete <name>")
			}
			os.Exit(1)
		}
	...
	default:
		...
		if !jsonMode {
			fmt.Println()
			fmt.Println("Usage: agent-desk profile <command>")
			fmt.Println()
...
func handleProfileList(out *CLIOutput, jsonMode bool) {
	...
	if len(profiles) == 0 {
		fmt.Println("No profiles found.")
		fmt.Println("Run 'agent-desk' to create the default profile automatically.")
		return
	}
...
func handleUpdate(args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk update [options]")
		fmt.Println()
		...
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  agent-desk update           # Check and install if available")
		fmt.Println("  agent-desk update --check   # Only check, don't install")
	}
	...
	if *checkOnly {
		fmt.Println("\nRun 'agent-desk update' to install.")
		return
	}
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("Usage: agent-desk [-p profile] [command]")
	fmt.Println()
	...
	fmt.Println("  uninstall        Uninstall Agent Desk")
	...
	fmt.Println()
	fmt.Println("Examples:")
	fmt.Println("  agent-desk                            # Start TUI with default profile")
	fmt.Println("  agent-desk -p work                    # Start TUI with 'work' profile")
	fmt.Println("  agent-desk add .                      # Add current directory")
	fmt.Println("  agent-desk add -t \"My App\" -g dev .   # With title and group")
	fmt.Println("  agent-desk session start my-project   # Start a session")
	fmt.Println("  agent-desk session show               # Show current session (in tmux)")
	fmt.Println("  agent-desk mcp list --json            # List MCPs as JSON")
	fmt.Println("  agent-desk mcp attach my-app exa      # Attach MCP to session")
	fmt.Println("  agent-desk group move my-app work     # Move session to group")
	fmt.Println()
	fmt.Println("Environment Variables:")
	fmt.Println("  AGENTDESK_PROFILE    Default profile to use")
	fmt.Println("  AGENTDESK_COLOR      Color mode: truecolor, 256, 16, none")
	fmt.Println()
...
// handleUninstall removes agent-desk from the system
func handleUninstall(args []string) {
	fs := flag.NewFlagSet("uninstall", flag.ExitOnError)
	keepData := fs.Bool("keep-data", false, "Keep ~/.agent-desk/ (sessions, config, logs)")
	...
	fs.Usage = func() {
		fmt.Println("Usage: agent-desk uninstall [options]")
		fmt.Println()
		fmt.Println("Uninstall Agent Desk from your system.")
		fmt.Println()
		fmt.Println("Options:")
		fmt.Println("  --dry-run           Show what would be removed without removing")
		fmt.Println("  --keep-data         Keep ~/.agent-desk/ (sessions, config, logs)")
		fmt.Println("  --keep-tmux-config  Keep tmux configuration")
		fmt.Println("  -y                  Skip confirmation prompts")
		fmt.Println()
		fmt.Println("Examples:")
		fmt.Println("  agent-desk uninstall              # Interactive uninstall")
		fmt.Println("  agent-desk uninstall --dry-run    # Preview what would be removed")
		fmt.Println("  agent-desk uninstall --keep-data  # Remove binary only, keep sessions")
		fmt.Println("  agent-desk uninstall -y           # Uninstall without prompts")
	}
	...
	fmt.Println("╔════════════════════════════════════════╗")
	fmt.Println("║       Agent Desk Uninstaller           ║")
	fmt.Println("╚════════════════════════════════════════╝")
	fmt.Println()
	...
	dataDir := filepath.Join(homeDir, ".agent-desk")
	...
	if _, err := exec.LookPath("brew"); err == nil {
		cmd := exec.Command("brew", "list", "agent-desk")
		if cmd.Run() == nil {
			homebrewInstalled = true
			foundItems = append(foundItems, foundItem{"homebrew", "", "Homebrew package: agent-desk"})
	...
	binaryLocations := []string{
		filepath.Join(homeDir, ".local", "bin", "agent-desk"),
		"/usr/local/bin/agent-desk",
		filepath.Join(homeDir, "bin", "agent-desk"),
	}
	...
	if data, err := os.ReadFile(tmuxConf); err == nil {
		if strings.Contains(string(data), "# agent-desk configuration") {
	...
	// Nothing found?
	if len(foundItems) == 0 {
		fmt.Println("Agent Desk does not appear to be installed.")
		fmt.Println()
		...
		fmt.Printf("  - %s\n", dataDir)
		fmt.Printf("  - %s (for agent-desk config)\n", tmuxConf)
		return
	}
	...
	for _, item := range foundItems {
		...
		case "homebrew":
			fmt.Println("  • Homebrew package: agent-desk")
	...
	if homebrewInstalled {
		...
		cmd := exec.Command("brew", "uninstall", "agent-desk")
	...
	for _, item := range foundItems {
		...
		testFile := filepath.Join(dir, ".agent-desk-write-test")
	...
	if !*keepTmuxConfig {
		for _, item := range foundItems {
			...
			// Create backup
			backupPath := tmuxConf + ".bak.agentdesk-uninstall"
			if err := os.WriteFile(backupPath, data, 0o644); err != nil {
				fmt.Printf("Warning: failed to create backup: %v\n", err)
			}

			// Remove the agent-desk config block
			content := string(data)
			startMarker := "# agent-desk configuration"
			endMarker := "# End agent-desk configuration"
			...
			if !*yes {
				...
				if strings.ToLower(response) != "n" {
					backupFile := filepath.Join(
						homeDir,
						fmt.Sprintf("agent-desk-backup-%s.tar.gz", time.Now().Format("20060102-150405")),
					)
					fmt.Printf("Creating backup at %s...\n", backupFile)

					cmd := exec.Command("tar", "-czf", backupFile, "-C", homeDir, ".agent-desk")
					if err := cmd.Run(); err != nil {
						fmt.Printf("Warning: failed to create backup: %v\n", err)
					} else {
	...
	if *keepData {
		...
		fmt.Println("      Remove manually with: rm -rf ~/.agent-desk")
	}

	if *keepTmuxConfig {
		fmt.Println("Note: tmux config preserved in ~/.tmux.conf")
		fmt.Println("      Remove the '# agent-desk configuration' block manually if desired")
	}

	fmt.Println()
	fmt.Println("Thank you for using Agent Desk!")
	fmt.Println("Feedback: https://github.com/millwright-software/agent-desk/issues")
}

// isNestedSession returns true if we're running inside an agent-desk managed tmux session.
// Uses GetCurrentSessionID() which checks if the current tmux session name matches agentdesk_*.
